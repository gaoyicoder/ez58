...
if(!submit_check(CURSCRIPT.'_submit')){
	...
	if(is_array($batchnew) && $step == 1){
		...
		if(empty($batchnew['cityname'])) write_msg('分站城市名称不能为空，多个城市请以空格分隔开');
	...
	if(is_array($citynew)){
		...
		empty($citynew['cityname']) && write_msg('城市分站名称不能为空！');
		empty($citynew['firstletter']) && write_msg('首字母不能为空！');
		empty($citynew['directory']) && write_msg('保存目录名称不能为空！');
		empty($citynew['citypy']) && write_msg('城市名全拼/英文全称不能为空！');
	...
	if(is_array($cityedit)) {
		...
		empty($cityedit['cityname']) && write_msg('城市分站名称不能为空！');
		empty($cityedit['firstletter']) && write_msg('首字母不能为空！');
		empty($cityedit['directory']) && write_msg('保存目录名称不能为空！');
		empty($cityedit['citypy']) && write_msg('城市名全拼/英文全称不能为空！');
	...
	if(is_array($newarea)){
		empty($newarea['cityid']) && write_msg('请选择隶属的城市分站！');
	...
	if(is_array($newstreet)){
		empty($newstreet['areaid']) && write_msg('请选择隶属的分站地区！');
