...
(!defined('IN_ADMIN') || !defined('IN_MYMPS')) && die('Access Denied');
$part = $part ? $part : 'list';
$cat_color = $color;
...
function info_typemodels($_var_0 = "")
{
	global $db, $db_mymps;
	$_var_1 = "SELECT id,name,displayorder FROM `{$db_mymps}info_typemodels` ORDER BY displayorder ASC";
	$_var_2 = $db->getAll($_var_1);
	foreach ($_var_2 as $_var_3 => $_var_4) {
		$_var_5 .= '<option value="' . $_var_4[id] . '"';
		$_var_5 .= $_var_0 == $_var_4[id] ? 'selected style="background-color:#6EB00C;color:white"' : "";
		$_var_5 .= '>' . $_var_4[name] . '</option>';
	}
	return $_var_5;
}