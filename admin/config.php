...
缓存设置
*/
elseif($part == 'cache_sys'){
	chk_admin_purview("purview_缓存设置");
	$here = '数据缓存';
	$cachearray = array(
				'write_admin_cache'=>'更新管理员缓存',
				'updateadvertisement'=>'更新广告缓存',
				'write_cron_cache'=>'更新定时任务缓存',
				'write_checkanswer_cache'=>'更新验证问答缓存',
				'update_checkanswer_settings'=>'更新验证问答设置缓存',
				'update_jswizard_settings'=>'更新数据调用设置缓存',
				'write_jswizard_cache'=>'更新数据调用项目缓存',
				'write_authcode_cache'=>'更新验证码设置缓存',
				'write_plugin_cache'=>'更新插件缓存',
				'write_insidelink_cache'=>'更新站内文字链接缓存',
				'write_qqlogin_cache'=>'更新QQ登录接口设置缓存'
				);
	include mymps_tpl("cache_sys");
}
/*更新页面缓存*/
elseif($part == 'cache_update'){
...
elseif($part == 'cacheupdate'){
	mymps_delete("cache");//清空表记录	
	clear_cache_files('cache');//清除缓存	
