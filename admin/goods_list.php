...
if(!submit_check(CURSCRIPT.'_submit')){
	...
	if($part == 'edit'){
		...
		if(empty($id)) write_msg('商品编号不能为空！');
		$edit = $db -> getRow("SELECT * FROM `{$db_mymps}goods` WHERE goodsid = '$id'");
		if(empty($edit['goodsid'])) write_msg('该商品已不存在！');
	...
	if($part == 'list'){
		...
		if(empty($selectedids)) write_msg('您没有选中任何一个商品！');
		...
		if(empty($id)) write_msg('商品编号不能为空！');
		if(empty($goodsname)) write_msg('商品名称不能为空！');
		if(empty($content)) write_msg('商品详情不能为空！');
