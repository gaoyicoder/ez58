...
if(!submit_check(CURSCRIPT.'_submit')){
	...
	if($part == 'edit'){
		...
		if(empty($id)) write_msg('团购活动编号不能为空！');
		$edit = $db -> getRow("SELECT * FROM `{$db_mymps}group` WHERE groupid = '$id'");
		if(empty($edit['groupid'])) write_msg('该团购活动已不存在！');
	...
	if($part == 'list'){
		...
		if(empty($selectedids)) write_msg('您没有选中任何一个团购活动！');
		...
		if(empty($id)) write_msg('团购活动编号不能为空！');
		if(empty($gname)) write_msg('活动名称不能为空！');
		if(empty($gaddress)) write_msg('活动地点不能为空！');
		if(empty($des)) write_msg('活动简短说明不能为空！');
		if(empty($meetdate)) write_msg('集合时间不能为空！');
		if(empty($enddate)) write_msg('截止时间不能为空！');
