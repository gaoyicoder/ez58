		...
		if($action == 'delall'){
			...
			switch($do){
				...
				case 'post':
					...
					if(empty($cityid)) write_msg('请选择发布所在分站！');
					...
					if(empty($contact_who)) write_msg("请填写联系人！");
					$k = $v = NULL;
					if(is_array($extra)){
			...
			if(empty($id)) write_msg('您没有选中任何一条信息记录');
