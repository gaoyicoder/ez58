...
if(!submit_check(CURSCRIPT.'_submit')) {
	...
	switch($part){
		...
		break;
		case 'detail':
			if(empty($id)) write_msg('很抱歉，没有该数据调用项目！');
	...
	if(empty($id)){
		//添加调用
		if(empty($flag) || !is_array($parameter)) write_msg('唯一标识不能为空！相关配置不能为空！');
		empty($parameter['jstemplate']) && write_msg('数据调用模板内容不能为空！');
		if($db -> getOne("SELECT count(id) FROM `{$db_mymps}jswizard` WHERE flag = '$flag'") > 0) write_msg('该标识已经存在，请更换一个唯一标识！');
