...
switch ($do){
	...
	case 'member':
		...
		if($part == 'default'){
			...
			if($do_action == 'delall') {
				...
				foreach ($id as $k => $v){
					...
					while($r = $db -> fetchRow($query)){
						if($r[modid] > 1 && $r[id]) mymps_delete("information_".$r[modid],"WHERE id = '$r[id]'");
					}
			...
			if(!$id) write_msg('用户不能为空！');
			if(!$userid) write_msg('用户名不能为空！');
			if(!$paymoney) write_msg('请输入要充值的金币数！');
