...
if ($part == 'chk_answer') {
	...
	die(mymps_chk_randcode($getvalue) ? 'success' : '验证码错误，请重新输入');
	$getvalue = '';
} elseif ($part == 'chk_smsauthcode') {
	@session_save_path(MYMPS_ROOT . '/data/sessions');
	die(mymps_chk_smsrandcode($getvalue) ? 'success' : '手机验证码输入不正确');
	$getvalue = '';
	...
	if (empty($getvalue)) {
		...
		if (PASSPORT_TYPE == 'phpwind') {
			...
			die(uc_user_get($getvalue) ? '很遗憾！该用户名已被注册！' : 'success');
		} elseif (PASSPORT_TYPE == 'ucenter') {
			include MYMPS_ROOT . '/uc_client/client.php';
			die(uc_get_user($getvalue) ? '很遗憾！该用户名已被注册！' : 'success');
		} else {
			$check = CheckUserID($getvalue, '用户名');
			if (strstr($getvalue, 'admin') || strstr($getvalue, '管理员')) {
				die('该用户名已被保护，请换一个用户名！');
			}
			if (strlen($getvalue) < 4 || strlen($getvalue) > 20) {
				die('可填写字母、数字、下划线_，不得少于4个字符');
			}
			if ($check == 'ok') {
				die(!($re = $db->getOne("SELECT * FROM {$db_mymps}member WHERE userid LIKE '{$getvalue}'")) ? 'success' : '很抱歉！该用户名已经被注册！');
			} else {
				die($check);
			}
		}
	...
	empty($id) && die(html2js('Invalid Id'));
	...
	if ($member_log->chk_in()) {
		...
		$return['s_uid'] = $charset == 'gbk' ? iconv('GB2312', 'UTF-8//IGNORE', $s_uid) : $s_uid;
	} else {
		$return['login'] = 'error';
	}
