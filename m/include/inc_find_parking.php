...
$idin = get_page_idin("id","SELECT a.id FROM `{$db_mymps}information` AS a {$s} WHERE (a.info_level = 1 OR a.info_level = 2) AND a.book_uid='' AND a.endtime > $timestamp {$sq}{$cate_limit}{$city_limit}{$orderby}",$perpage);
