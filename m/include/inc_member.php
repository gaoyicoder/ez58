...
if ($dopost == 1) {
	switch ($action) {
		...
		case 'editbase':
			...
			break;
		case 'editpwd':
			empty($userpwd) && redirectmsg('请输入新密码！', 'index.php?mod=member&action=editpwd');
			if (strlen($userpwd) < 6 || strlen($userpwd) > 15) {
				redirectmsg('密码不能小于6位或超过15位！', 'index.php?mod=member&action=editpwd');
			}
			if ($reuserpwd != $userpwd) {
				redirectmsg('两次密码输入不相同！', 'index.php?mod=member&action=editpwd');
			}
		...
		case 'pay':
			...
			if ($money) {
				...
				if ($money < 1) {
					redirectmsg('充值的数量不能小于1', 'javascript:history.back();');
				}
				$payid = (int) $payid;
				if (!$payid) {
					redirectmsg('请选择支付接口', 'javascript:history.back();');
				}
				$payr = $db->getRow("SELECT * FROM {$db_mymps}payapi WHERE payid='{$payid}' AND isclose=0");
				if (!$payr['payid']) {
					redirectmsg('该支付接口未开启', 'javascript:history.back();');
				}
				$ddno = $timestamp;
				$pay_type = 'PayToMoney';
				$productname = '金币充值';
			...
			break;
		case 'mypost':
			$row = $db->getRow("SELECT money_own FROM `{$db_mymps}member` WHERE userid = '{$s_uid}'");
			if ($act == 'refreshid') {
				if (!$id) {
					echo '该信息不存在！';
				} else {
					...
					if ($delmoney) {
						$db->query("UPDATE `{$db_mymps}member` SET money_own = money_own - '{$delmoney}' WHERE userid = '{$s_uid}'");
					}
				...
				if (!$id) {
					...
					if ($r[modid] > 1) {
						mymps_delete('information_' . $r[modid], "WHERE id = '{$id}'");
					}
	...
	switch ($action) {
		case 'upface':
			include mymps_tpl('member_upface');
			break;
		case 'editbase':
			include mymps_tpl('member_editbase');
			break;
		case 'editpwd':
			include mymps_tpl('member_editpwd');
			break;
		case 'pay':
		...
		case 'shoucang':
			...
			break;
		case 'docu':
			$row = $db->getRow("SELECT id FROM `{$db_mymps}member` WHERE userid = '{$s_uid}'");
